...
public class BasicShape implements Shape {
  ...
  private TotalSize currDimensions;

  private TreeMap<ChangeTime, ICommands> commands;
  ...
  @Override
  public Color getCurrColor() throws IllegalArgumentException {
    checkNull(this.currColor, "Current Color not initialized");
    return this.currColor;
  }

  @Override
  public Position getCurrentPosition() throws IllegalArgumentException {
    checkNull(this.currPos, "Current Position not initialized");
    return this.currPos;
  }

  @Override
  public TotalSize getCurrentDimension() throws IllegalArgumentException {
  ...
  public List<ICommands> getCommands() throws IllegalArgumentException {
    ...
    for (Entry<ChangeTime, ICommands> entry : commands.entrySet()) {
  ...
  public void addCommand(ICommands com) throws IllegalArgumentException {
    ...
    if (com.getShape().equals(name)) {
      ChangeTime changeTimeCom = com.getTicks();
   ...
   */
  private void checkOverlap(ChangeTime cCommand) {
    int newStart = cCommand.getStart();
    int newEnd = cCommand.getEnd();
    for (ChangeTime t : this.commands.keySet()) {
   ...
   */
  private void checkConsistancy(ICommands comToAdd, ChangeTime timeToAdd) {
    for (ChangeTime t : this.commands.keySet()) {
  ...
  public boolean compareTick(int tick) {
    ...
    for (ChangeTime com : this.commands.keySet()) {
  ...
  public void checkGaps() {
    ...
    List<ChangeTime> listOfChangeTimes = new ArrayList<>();
    ...
    for (int i = 0; i < listOfChangeTimes.size() - 1; i++) {
      ...
      if (listOfChangeTimes.get(i + 1).getStart() - listOfChangeTimes.get(i).getEnd() > 0) {
        ...
        ChangeTime changeTimeToAdd = new ChangeTime(listOfChangeTimes.get(i).getEnd(),
  ...
  public void updateShape(int tick) {
    for (ChangeTime time : this.commands.keySet()) {
   ...
   */
  private boolean inBetweenAndOutOfDate(int tick, ChangeTime time) {
