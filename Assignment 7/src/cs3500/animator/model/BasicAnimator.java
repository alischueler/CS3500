...
public final class BasicAnimator implements IAnimator<Shape> {
  ...
  private Map<String, List<ICommands>> commands;
   ...
   */
  private BasicAnimator(List<Shape> shapes, Map<String, List<ICommands>> commands,
      cs3500.animator.model.SimpleBackground background) {
    if (background == null) {
  ...
  public static final class Builder implements AnimationBuilder<IAnimator<Shape>> {
    ...
    private List<cs3500.animator.model.Shape> shapes = new ArrayList<>();
    private Map<String, List<ICommands>> commands = new HashMap<>();
    private cs3500.animator.model.SimpleBackground background;
    ...
    public AnimationBuilder<IAnimator<Shape>> setBounds(int x, int y, int width, int height) {
      this.background = new cs3500.animator.model.SimpleBackground(height, width, x, y);
      return this;
    }
    ...
    public AnimationBuilder<IAnimator<Shape>> declareShape(String name, String type) {
      if (name == null || type == null) {
        throw new IllegalArgumentException("name of type is null");
      }
    ...
    @Override
    public AnimationBuilder<IAnimator<Shape>> addMotion(String name, int t1, int x1, int y1, int w1,
    ...
    for (cs3500.animator.model.Shape s : allShapes) {
      for (String name : commands.keySet()) {
        if (s.getShapeName().equals(name)) {
          for (ICommands com : commands.get(name)) {
      ...
      for (Shape s : allShapes) {
        if (s.getShapeName().equals(name)) {
          ...
          if (commands.containsKey(name)) {
            ...
            List<ICommands> newList = new ArrayList<>();
            newList.add(command);
  ...
  public void addShape(Shape s) throws IllegalArgumentException,
    ...
    if (!commands.containsKey(s.getShapeName())) {
      ...
      for (ICommands command : commands.get(s.getShapeName())) {
        s.addCommand(command);
        s.checkGaps();
      }
    }
  ...
  private boolean checkAllValues(ICommands commandToRemove) {
    ...
    if (commands.containsKey(commandToRemove.getShape())) {
      for (ICommands existingCommand : commands.get(commandToRemove.getShape())) {
