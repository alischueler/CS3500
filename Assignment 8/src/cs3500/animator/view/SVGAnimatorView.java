...
import cs3500.animator.model.IChangeTime;
...
public class SVGAnimatorView implements IAnimatorView {
  ...
  protected final IViewModel<Shape> animator;
  private int tempo;
  ...
  void makeSVGForCommand(StringBuilder soFar, ICommands command, String loopBackString, String type,
    ...
    IPosition startPos = command.getStartPosition();
    IPosition endPos = command.getEndPosition();
    ITotalSize startSize = command.getStartSize();
    ITotalSize endSize = command.getEndSize();
    ...
    if (type.equals("rectangle")) {
      ...
      makeAnimationCommand(soFar, begin, dur, "cy",
          transform(startPos.getY(), yBound), transform(endPos.getY(), yBound), fill);
      makeAnimationCommand(soFar, begin, dur, "ry", transformDouble(startSize.getTotalHeight() / 2),
          transformDouble(endSize.getTotalHeight() / 2), fill);
      makeAnimationCommand(soFar, begin, dur, "rx", transformDouble(startSize.getTotalWidth() / 2),
          transformDouble(endSize.getTotalWidth() / 2),
          fill);
      makeAnimationCommand(soFar, begin, dur, "fill", makeColor(command.getStartColor()),
          makeColor(command.getEndColor()), fill);
    }
   ...
   */
  protected String transform(double ogVal, int canvasVal) {
   ...
   */
  protected String transformDouble(double ogVal) {
   ...
   */
  protected void makeAnimationCommand(StringBuilder soFar, String begin, String dur, String type,
   ...
   */
  protected String getDurTime(IChangeTime c) {
   ...
   */
  protected String getStartTime(IChangeTime c) {
   ...
   */
  protected String makeColor(IColor c) {
   ...
   */
  protected void initializeShapeSVGHelper(StringBuilder soFar, String type, String name, String x,
  ...
  void initializeFile(StringBuilder soFar) {
    IBackground background = this.animator.getBackground();
