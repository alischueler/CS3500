...
CHANGE ASSIGNMENT 6: we added a methods for removing commands and shapes and adding commands and
shapes and a
...
CHANGE ASSIGNMENT 6: we now represent commands as their own class Commands to store and access them
more efficiently,
...
in gaps of commands
CHANGE ASSIGNMENT 6: we added methods to check commands to the shape, this would make our design
easier to extend
...
CHANGE ASSIGNMENT 7: we added a new enum interactive and added it to the switch statement as well to
 ...
 CHANGE ASSIGNMENT 8: we added a new enum provider and added it to the switch statement as well to
 account for the new provider's interactive vire for this assignment.

Controller Creator:
creates a controller using the view enum it was given as well as a model. Will create either
controller, visual controller, or interactive controller
CHANGE ASSIGNMENT 8: we added the option to create a provider controller if given the provider view
enum.
...
ProviderController:
a controller to visualize the provider's interactive view and functionality. Extends Controller and
not InteractiveController because the providers have the timer in their view so having another
timer in the controller would have been unnecessary.
...
IN ASSIGMENT 8:
  added interfaces for ChangeTime, Color, ChangeTime, TotalSize.
  This was done to avoid sending the customers some of our class implementations.

IChangeTime:
Holds information on a tick. Added in assigment 8.

IColor:
Holds information on a color. provides setters and getters for the color data. Added im assigment 8.

IChangeTime:
Holds information on a IPosition. provides setters and getter for accessing the position
information. Added in assigment 8.

ITotalSize:
Holds information on a the size of a shape. provides setters and getters for the data it holds.
Added is assigment 8.