...
public class BasicAnimator implements IAnimator<Shape> {
  ...
  public void startAnimator(Map<String, List<String>> commands) throws IllegalArgumentException {
    ...
    for (Map.Entry<String, List<String>> entry: mapCommands.entrySet()) {
      Shape shapeToAdd = new BasicShape(entry.getKey(),
          entry.getValue().get(0).split(" ")[1]);
      currentShapes.put(entry.getKey(), shapeToAdd);
      allShapes.add(shapeToAdd);
    }
    //mutate the shapes in the map to their first start and end points
    for (Shape s : allShapes) {
      for (int i = 0; i < mapCommands.get(s).size(); i++) {
        mutateEntireShape(mapCommands.get(s).get(i), s);
      }
    }
  }
  ...
  private void isAnimationStarted() throws IllegalStateException {
    if (this.mapCommands == null && this.allShapes == null && this.currentShapes == null) {
  ...
  private void mutateOutOfDateShape(int tick, String shapeName) throws IllegalArgumentException {
    ...
    List<String> commands = this.mapCommands.get(shapeName);
  ...
  private void mutateEntireShape(String command, Shape shapeToBeMutated)
    ...
    //adding this information to the shapes history
    shapeToBeMutated.addToInfo();
  }
